...
from dq_etl_starter.exporters import export_cleaned_csv, export_to_postgres, export_to_sqlite
...
from dq_etl_starter.report import build_quality_report, export_report_json, export_report_markdown
...
def read_input(config: WorkflowConfig):
    ...
    if config.input_type == "json":
        return read_json_file(config.input_path, records_path=config.records_path)
    if config.input_type == "mock-api":
        return mock_api_response_to_dataframe(config.input_path, records_path=config.records_path or "data.orders")
...
def run_workflow(config: WorkflowConfig):
    ...
    output_files = {"cleaned_csv": str(cleaned_csv_path)}

    if config.db_target == "sqlite":
        sqlite_path = config.output_dir / "etl_output.sqlite"
        export_to_sqlite(cleaned_df, sqlite_path, config.table_name)
        output_files["sqlite"] = str(sqlite_path)
    elif config.db_target == "postgres":
        export_to_postgres(cleaned_df, config.table_name, database_url=config.database_url)
        output_files["postgres_table"] = config.table_name
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run a small data quality ETL workflow.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    run_parser = subparsers.add_parser("run", help="Run the ETL workflow")
    run_parser.add_argument("--input", required=True, dest="input_path")
    run_parser.add_argument("--input-type", required=True, choices=["csv", "excel", "json", "mock-api"])
    run_parser.add_argument("--schema", required=True, dest="schema_path")
    run_parser.add_argument("--output-dir", required=True)
    run_parser.add_argument("--db-target", default="sqlite", choices=["none", "sqlite", "postgres"])
    run_parser.add_argument("--table-name", default="cleaned_data")
    run_parser.add_argument("--database-url", default=None)
    run_parser.add_argument("--records-path", default=None, help="Dot path to records for nested JSON or mock API payloads")
    return parser
